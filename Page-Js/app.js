(function () {
    ...
    document.addEventListener("click", (event) => {
      ...
      if (action === "toggle-password") {
        ...
        togglePasswordVisibility(
          target.getAttribute("data-password-input") || "",
          target,
        );
        return;
      }
  ...
  function handleScrollVisibility() {
    dom.scrollTopButton?.classList.toggle(
      "scroll-top-visible",
      window.scrollY > 120,
    );
  }
  ...
  function updateInstallButton() {
    ...
    dom.installAppBtn.classList.toggle(
      "hidden",
      !deferredInstallPrompt && !canShowFallback,
    );
    dom.installAppBtn.textContent = deferredInstallPrompt
      ? "Install app"
      : "Add to home screen";
  }
  ...
  function toggleLandingMenu(forceOpen) {
    ...
    dom.landingMenuToggle?.setAttribute(
      "aria-expanded",
  ...
  function toggleAppSidebar(forceOpen) {
    ...
    dom.appSidebarToggle?.setAttribute(
      "aria-expanded",
      nextState ? "true" : "false",
    );
    setOverlayLockState();
  ...
  function seedStorage() {
    if (!localStorage.getItem(COMMUNITY_KEY)) {
      localStorage.setItem(
        COMMUNITY_KEY,
        JSON.stringify(window.RKH_DATA.demoMessages),
      );
    }
  }

  function hydrateState() {
    state.communityMessages = readJson(
      COMMUNITY_KEY,
      window.RKH_DATA.demoMessages,
    )
    ...
    if (navState) {
      ...
      state.currentLessonVideoIndex =
        Number(navState.currentLessonVideoIndex) || 0;
      state.currentLiveClassId = navState.currentLiveClassId || null;
      state.currentCurriculumSemesterId =
        navState.currentCurriculumSemesterId || null;
      state.currentCurriculumMonthId =
        navState.currentCurriculumMonthId || null;
      state.currentResourcesSemesterId =
        navState.currentResourcesSemesterId || null;
    }
  }
  ...
  function normalizeUser(user) {
    return {
      ...user,
      completedLessonIds: Array.isArray(user.completedLessonIds)
        ? user.completedLessonIds
        : [],
      joinedClassIds: Array.isArray(user.joinedClassIds)
        ? user.joinedClassIds
        : [],
      assessmentSubmissions:
        user.assessmentSubmissions &&
        typeof user.assessmentSubmissions === "object"
          ? user.assessmentSubmissions
          : {},
  ...
  function setCommunityMessages(messages, options = {}) {
    ...
    const previousSignature = state.communityMessages
  ...
  function normalizeCommunityMessage(message) {
    const payload = parseCommunityPayload(
    ...
    const trackId =
      ...
      message.track_id ||
      message.room ||
      inferTrackId(message.authorTrack || message.author_track);
    const authorTrack =
      message.authorTrack ||
      message.author_track ||
      window.RKH_DATA.tracks[trackId]?.label ||
      "Community";
    const authorName =
      message.authorName ||
      message.author_name ||
  ...
  function getTrackAnnouncements(track) {
    ...
    return [...(track.announcements || [])].sort(
      (left, right) =>
        toTimestamp(right.createdAt) - toTimestamp(left.createdAt),
    );
  }
  ...
  function getYouTubeVideoId(url) {
    ...
    const directMatch = trimmed.match(
      /(?:youtu\.be\/|youtube(?:-nocookie)?\.com\/(?:embed\/|shorts\/|live\/))([a-zA-Z0-9_-]{11})/i,
    );
  ...
  function normalizeMediaUrlForPlayer(url) {
    if (!url) return "";
    const trimmed = String(url).trim();
    if (!trimmed) return "";
  ...
  function isDirectVideoUrl(url) {
    const normalized = normalizeMediaUrlForPlayer(url);
    if (!normalized) return false;

    try {
      const parsed = new URL(normalized);
      const pathname = parsed.pathname.toLowerCase();
      return /\.(mp4|webm|ogg|mov|m4v|m3u8|mpd)(\?.*)?$/i.test(pathname);
    } catch (error) {
      return /\.(mp4|webm|ogg|mov|m4v|m3u8|mpd)(\?.*)?$/i.test(
        String(url || "")
          .trim()
          .toLowerCase(),
      );
  ...
  function buildLessonMediaPlayerHtml(url, title = "") {
    const normalizedUrl = normalizeMediaUrlForPlayer(url);
    if (!normalizedUrl) {
      return '<div class="empty-state">No video is available for this lesson yet.</div>';
    }

    if (isDirectVideoUrl(normalizedUrl)) {
      return `
        <div class="lesson-video-direct-player">
          <video controls playsinline preload="metadata" src="${escapeAttribute(normalizedUrl)}" title="${escapeAttribute(title || "Lesson video")}">
    ...
    return `
      <div class="iframe-wrap lesson-watch-frame">
        <iframe src="${escapeAttribute(normalizedUrl)}" title="${escapeAttribute(title || "Lesson video")}" allowfullscreen loading="lazy"></iframe>
      </div>
    `;
  }
    ...
    const normalized = resolved
      .map((item, index) => {
        if (typeof item === "string") {
          ...
          return trimmed
            ? { title: `Video ${index + 1}`, url: toEmbedUrl(trimmed) }
            : null;
        }

        if (item && typeof item === "object") {
          const url = String(
            item.url || item.videoUrl || item.video_url || "",
          ).trim();
          if (!url) return null;
          return {
  ...
  function parseCommunityPayload(value) {
    ...
    try {
      ...
      if (
        ...
        ("body" in parsed || "sticker" in parsed || "attachment" in parsed)
      ) {
        return {
  ...
  function buildFallbackTrack(trackId, trackSettings = {}) {
    ...
    for (let s = 1; s <= totalSemesters; s++) {
      ...
      for (let m = 1; m <= 4; m++) {
        ...
        months.push({
          ...
          summary:
            m === 4
              ? "A practical lab month focused on guided build work, testing, and final showcase delivery."
              : `Curriculum block for Month ${m}.`,
    ...
    return {
      ...
      summary:
        trackSettings.summary ||
        "Custom programme track created from the admin dashboard.",
      outcomes:
        Array.isArray(trackSettings.outcomes) && trackSettings.outcomes.length
  ...
  function getAvailableTracks() {
    ...
    const enabledTracks = resolvedTracks.filter(
  ...
  function getResolvedTrack(trackId) {
    ...
    const effectiveSemesterCount = Number.isFinite(trackSettings.semesterCount)
      ? trackSettings.semesterCount
      : null;
    ...
    if (
      ...
      baseTrack.semesters.length !== effectiveSemesterCount
    ) {
      baseTrack = {
        ...baseTrack,
        semesters: buildFallbackTrack(trackId, {
          semesterCount: effectiveSemesterCount,
        }).semesters,
      };
    } else if (!baseTrack) {
    ...
    return {
      ...
      sortOrder: Number.isFinite(trackSettings.sortOrder)
        ? trackSettings.sortOrder
        : 0,
      semesterCount:
        effectiveSemesterCount ?? (baseTrack.semesters?.length || 3),
      semesters: baseTrack.semesters.map((semester) => ({
        ...semester,
        months: semester.months.map((month) => {
          const monthOverride =
            state.curriculumMonthOverridesById[month.id] || {};
          return {
            ...month,
            ...
            weeks: month.weeks.map((week) => {
              const weekOverride =
                state.curriculumWeekOverridesById[week.id] || {};
              ...
              const resourceItems = normalizeLessonResourceItems(
                Array.isArray(weekOverride.resourceItems) &&
                  weekOverride.resourceItems.length
                  ? weekOverride.resourceItems
                  : Array.isArray(weekOverride.resources) &&
                      weekOverride.resources.length
              ...
              return {
                ...
                videoUrl:
                  videoItems[0]?.url || weekOverride.videoUrl || week.videoUrl,
  ...
  function getFounderUser() {
    ...
    if (
      user &&
      user.email &&
      user.email.toLowerCase() === FOUNDER_PROFILE.email.toLowerCase()
    ) {
      return user;
    }
    return null;
  ...
  function renderFounderShowcase() {
    ...
    const founderBio =
      founderUser?.bio?.trim() ||
      founderUser?.headline?.trim() ||
      FOUNDER_PROFILE.bio;
  ...
  function ensureCurriculumDefaults(track) {
    ...
    if (
      !allSemesters.some(
        (semester) => semester.id === state.currentCurriculumSemesterId,
      )
    ) {
    ...
    if (
      !allMonths.some((month) => month.id === state.currentCurriculumMonthId)
    ) {
  ...
  function ensureCommunityFeedForTrack(track) {
    ...
    if (!communitySupabase) {
      ...
      const localMessages = readStoredCommunityMessages().filter(
        (message) => message.trackId === track.id,
      );
  ...
  function applyAuthenticatedSession(session) {
    ...
    const fallbackTrackId =
      window.RKH_AUTH_HELPERS?.resolveEffectiveTrackId(
        session?.user?.user_metadata || {},
        availableTrackIds,
      ) || "";
    const userSnapshot =
      window.RKH_AUTH_HELPERS?.buildAuthenticatedUserSnapshot(
        session,
        fallbackTrackId,
      ) || null;
  ...
  function initializeCommunitySync() {
    ...
    communitySupabase = window.supabase.createClient(
      SUPABASE_URL,
      SUPABASE_ANON_KEY,
      {
        auth: {
    ...
    communitySupabase.auth.onAuthStateChange(async (event, session) => {
      ...
      const action =
        window.RKH_AUTH_HELPERS?.resolveAuthEventAction(
          event,
          session,
          lastObservedAuthUserId,
        ) || "keep-current";
      const sessionUserId = session?.user?.id || null;

      if (
        action === "show-landing" &&
        sessionUserId !== lastObservedAuthUserId
      ) {
  ...
  async function refreshCommunityMessages(trackId, options = {}) {
    ...
    if (!communitySupabase) {
      const localMessages = readStoredCommunityMessages().filter(
        (message) => message.trackId === trackId,
      );
  ...
  async function refreshTrackSettings(options = {}) {
    ...
    const { data, error } = await communitySupabase
      ...
      .select(
        "id, label, summary, outcomes, is_enabled, sort_order, semester_count",
      )
    ...
    state.trackSettingsById = Object.fromEntries(
      (data || []).map((item) => [
        item.id,
        {
          label: item.label || "",
  ...
  async function refreshCurriculumOverrides(options = {}) {
    ...
    const [
      ...
      communitySupabase
        ...
        .select(
          "week_id, title, objective, type, video_url, video_urls, resources, resource_items",
        ),
    ]);
    ...
    if (monthError) {
      ...
      state.curriculumMonthOverridesById = Object.fromEntries(
        (monthData || []).map((item) => [
          item.month_id,
          {
            label: item.label || "",
    ...
    if (weekError) {
      ...
      state.curriculumWeekOverridesById = Object.fromEntries(
        (weekData || []).map((item) => [
          item.week_id,
          {
            title: item.title || "",
  ...
  async function refreshSemesterResources(options = {}) {
    ...
    state.semesterResourcesByKey = Object.fromEntries(
      (data || []).map((item) => [
  ...
  async function fetchRemotePublicProfile(email) {
    ...
    const { data, error } = await communitySupabase.rpc(
      "get_lms_public_profile",
      {
        profile_email: email,
      },
    );
    ...
    return {
      ...
      completedLessonIds: Array.isArray(profile.completed_lesson_ids)
        ? profile.completed_lesson_ids
        : [],
      joinedClassIds: Array.isArray(profile.joined_class_ids)
        ? profile.joined_class_ids
        : [],
  ...
  async function syncUserProfileToRemote(user) {
    if (!communitySupabase || !user?.email || !remoteProfileSyncHealthy)
      return null;
  ...
  async function syncCurrentUserProfileFromRemote() {
    ...
    user.completedLessonIds =
      remoteProfile.completedLessonIds || user.completedLessonIds;
    user.joinedClassIds = remoteProfile.joinedClassIds || user.joinedClassIds;
    user.lastSeenCommunityAt =
      remoteProfile.lastSeenCommunityAt || user.lastSeenCommunityAt;
    user.lastSeenAnnouncementsAt =
      remoteProfile.lastSeenAnnouncementsAt || user.lastSeenAnnouncementsAt;
    user.certificateIssuedAt =
      remoteProfile.certificateIssuedAt || user.certificateIssuedAt;
    ...
    if (remoteProfile.isActive === false) {
      state.currentUserId = null;
      showAuthMessage(
        "This learner account has been disabled by the academy admin.",
        "error",
      );
      showAuthPage("login");
      return false;
  ...
  function handleRemoteProfileSyncError(error, action) {
    ...
    const isSchemaMismatch =
      ...
      /ambiguous/i.test(details);

    if (isSchemaMismatch) {
      remoteProfileSyncHealthy = false;
      if (!remoteProfileSyncWarningShown) {
        console.info(
          "Learner profile sync has been paused until the Supabase profile function is updated.",
          { action },
        );
  ...
  async function installRkhApp() {
    ...
    if (isIosInstallCandidate()) {
      window.alert(
        "To install RealKingHubs Academy on iPhone: tap Share in Safari, then choose Add to Home Screen.",
      );
      return;
    }
    ...
    if (isAndroidInstallFallbackCandidate()) {
      window.alert(
        "To install RealKingHubs Academy on your phone: open the browser menu, then choose Install app or Add to Home screen.",
      );
      return;
    }
  ...
  function isIosInstallCandidate() {
    const isAppleMobileDevice = /iphone|ipad|ipod/i.test(
  ...
  function switchAuthMode(mode) {
    ...
    if (
      dom.loginForm?.dataset.busy === "true" ||
      dom.registerForm?.dataset.busy === "true" ||
      dom.forgotPasswordForm?.dataset.busy === "true" ||
      dom.resetPasswordForm?.dataset.busy === "true"
    ) {
      return;
    }
    ...
    if (dom.forgotTab)
      dom.forgotTab.classList.toggle("auth-tab-active", forgotActive);
    if (dom.resetTab)
      dom.resetTab.classList.toggle("auth-tab-active", resetActive);

    if (dom.forgotTab)
      dom.forgotTab.classList.toggle("auth-tab-hidden", !forgotActive);
    if (dom.resetTab)
      dom.resetTab.classList.toggle("auth-tab-hidden", !resetActive);
    dom.loginTab.classList.toggle(
      "auth-tab-hidden",
      forgotActive || resetActive,
    );
    dom.registerTab.classList.toggle(
      "auth-tab-hidden",
      forgotActive || resetActive,
    );
  ...
  async function handleForgotPassword(event) {
    ...
    try {
      const { error } = await communitySupabase.auth.resetPasswordForEmail(
        email,
        {
  ...
  async function handleRegister(event) {
    ...
    const timezone =
      document.getElementById("registerTimezone").value.trim() ||
      "Africa/Lagos";
    const headline =
      document.getElementById("registerHeadline").value.trim() ||
      "Learner at RealKingHubs Academy";
    const password = document.getElementById("registerPassword").value;

    console.log("handleRegister: submit", {
      firstName,
      lastName,
      email,
      trackId,
    ...
    if (!firstName || !lastName || !email || !trackId || password.length < 8) {
      showAuthMessage(
        "Complete every registration field and use a password with at least 8 characters.",
        "error",
      );
      return;
    }
  ...
  function renderSidebar(user, track) {
    ...
    const navGroups = [
      {
        title: "Learning",
        items: ["dashboard", "curriculum", "resources", "books", "progress"],
      },
      { title: "Community", items: ["community", "announcements", "feedback"] },
      { title: "Account", items: ["certificates", "profile"] },
    ];

    dom.appNav.innerHTML = navGroups
      .map((group) => {
        const buttons = group.items
          .map((itemId) =>
            window.RKH_DATA.navItems.find((item) => item.id === itemId),
          )
          .filter(Boolean)
          .map(
            (item) => `
  ...
  function renderTopbar(user, track) {
    ...
    document
      .querySelector(".topbar-alert-button")
      ?.classList.toggle("topbar-alert-active", totalAlerts > 0);
  }
  ...
  async function renderCurrentView(user, track) {
    ...
    if (["community", "announcements"].includes(state.currentView))
      markSectionSeen(state.currentView);
  }
  ...
  function calculateTrackProgress(user, track) {
    const allLessons = track.semesters.flatMap((semester) =>
      semester.months.flatMap((month) => month.weeks),
    );
  ...
  function getCertificateData(user, track) {
    ...
    return {
      ...
      issuedAt,
      certificateId: `${track.id}-${user.id}`
        .replace(/[^a-z0-9-]/gi, "")
        .toUpperCase(),
      progress,
    };
  ...
  function getUnreadCommunityCount(user) {
    ...
    return state.communityMessages.filter(
      (message) =>
        message.authorEmail !== user.email &&
        toTimestamp(message.createdAt) > seenAt,
    ).length;
  }

  function getUnreadAnnouncementCount(user, track) {
    const seenAt = toTimestamp(user.lastSeenAnnouncementsAt);
    return getTrackAnnouncements(track).filter(
      (item) => toTimestamp(item.createdAt) > seenAt,
    ).length;
  }
  ...
  function renderDashboard(user, track) {
    ...
    const latestCommunityMessage =
      state.communityMessages.find((message) => message.trackId === track.id) ||
      null;
    const activeSemester =
      track.semesters.find(
        (semester) => semester.id === state.currentCurriculumSemesterId,
      ) || track.semesters[0];
    const monthFour =
      activeSemester?.months?.find((month) => month.label === "Month 4") ||
      null;
    const semesterCards = track.semesters
      .map((semester) => {
  ...
  function buildContentSurfaceHeader({
    eyebrow,
    title,
    description,
    metaItems = [],
  ...
  function renderCurriculum(user, track) {
    ...
    const openSemesterId =
      state.currentCurriculumSemesterId &&
      track.semesters.some(
        (semester) => semester.id === state.currentCurriculumSemesterId,
      )
        ? state.currentCurriculumSemesterId
        : null;
    const openMonthId = allMonths.some(
      (month) => month.id === state.currentCurriculumMonthId,
    )
      ? state.currentCurriculumMonthId
      : null;
    const semestersHtml = track.semesters
      .map((semester) => {
        const semesterProgress = calculateSemesterProgress(user, semester);
  ...
  function renderMonthCard(user, month, isOpen) {
    const completedWeeks = month.weeks.filter((week) =>
      user.completedLessonIds.includes(week.id),
    ).length;
    const monthPercent = month.weeks.length
      ? Math.round((completedWeeks / month.weeks.length) * 100)
      : 0;
    return `
  ...
  function renderLessonRow(user, lesson) {
    ...
    const videoCount =
      Array.isArray(lesson.videoItems) && lesson.videoItems.length
        ? lesson.videoItems.length
        : (lesson.videoUrls && lesson.videoUrls.length) ||
          (lesson.videoUrl ? 1 : 0);
    const videoBadge =
      videoCount > 0
  ...
  function renderCommunity(user, track) {
    const visibleMessages = state.showOlderMessages
      ? state.communityMessages
      : state.communityMessages.slice(0, 5);
    const remaining = Math.max(
      state.communityMessages.length - visibleMessages.length,
      0,
    );
    return `
  ...
  function renderMessageRow(user, message) {
    const ownMessage =
      user.id === message.authorId || user.email === message.authorEmail;
  ...
  function renderCommunityAttachment(attachment) {
    ...
    if (attachment.kind === "folder") {
      ...
      const folderLabel = escapeHtml(
        attachment.folderName ||
          attachment.name.replace(/\.zip$/i, "") ||
          "Shared folder",
      );
  ...
  function renderProgress(user, track) {
    ...
    const certificate = getCertificateData(user, track);
    const semesterRows = track.semesters
      .map((semester) => {
    ...
    const monthRows = track.semesters
      .flatMap((semester) => semester.months)
      .map((month) => {
        const completed = month.weeks.filter((week) =>
          user.completedLessonIds.includes(week.id),
        ).length;
        const percent = month.weeks.length
          ? Math.round((completed / month.weeks.length) * 100)
          : 0;
    ...
    return `
      <section class="dashboard-stack books-page-shell">
        ...
        `,
                ...
                .join("")}</div>`
            : '<div class="empty-state">No books have been added yet. Ask the admin to create a book card from the admin dashboard.</div>'
        }
    ...
    return `
      <section class="surface-card">
        ${buildContentSurfaceHeader({
          ...
          title: "Announcements",
          description:
            "Course notices, lab guidance, and learning updates live here instead of crowding the dashboard.",
          metaItems: [
        ...
        <article class="settings-card">
          ...
          <form id="profileForm" class="dashboard-stack">
            ...
            <div class="field-row"><div class="field-group"><label for="profileTrack">Track</label><select id="profileTrack">${getResolvedTracks()
              .map(
                (item) =>
                  `<option value="${item.id}" ${item.id === user.trackId ? "selected" : ""}>${item.label}</option>`,
              )
              .join(
                "",
              )}</select></div><div class="field-group"><label for="profileTimezone">Timezone</label><input id="profileTimezone" type="text" value="${escapeAttribute(user.timezone)}" /></div></div>
  ...
  async function saveProfileSettings(event) {
    ...
    const headline =
      ...
      "";
    const bio =
      document.getElementById("profileBio")?.value.trim() || user.bio || "";
    const trackId =
      document.getElementById("profileTrack")?.value || user.trackId;
    const timezone =
      document.getElementById("profileTimezone")?.value.trim() ||
      user.timezone ||
      "";
  ...
  async function finalizeProfileSave() {
    ...
    const profileForm = document.getElementById("profileForm");
    try {
      setAuthFormBusy(profileForm, false);
    } catch (e) {}
  }
  ...
  async function readFeedbackImageFiles(fileList) {
    ...
    const oversized = files.find(
      (file) => file.size > FEEDBACK_IMAGE_MAX_BYTES,
    );
    if (oversized) {
      ...
      files.map(
        (file) =>
          new Promise((resolve, reject) => {
            ...
            reader.onerror = () =>
              reject(
                reader.error || new Error("Could not read the selected image."),
              );
  ...
  async function submitFeedbackForm(event) {
    ...
    const message =
      document.getElementById("feedbackMessageText")?.value.trim() || "";
    const feedbackMessage = document.getElementById("feedbackMessage");
    const imageFiles =
      document.getElementById("feedbackImageInput")?.files || null;

    if (!message) {
      feedbackMessage.className = "form-message error";
      feedbackMessage.textContent =
        "Please write a short message before sending feedback.";
    ...
    try {
      imageUrls = imageFiles?.length
        ? await readFeedbackImageFiles(imageFiles)
        : [];
    } catch (error) {
      feedbackMessage.className = "form-message error";
      feedbackMessage.textContent =
        error?.message || "The selected images could not be attached.";
      feedbackMessage.classList.remove("hidden");
      return;
    }
    ...
    try {
      if (communitySupabase) {
        ...
        const stored = JSON.parse(
          localStorage.getItem("rkh_feedback_submissions") || "[]",
        );
        stored.unshift({ id: `feedback-${Date.now()}`, ...payload });
        localStorage.setItem(
          "rkh_feedback_submissions",
  ...
  function toggleCommunityStickerPack(forceOpen) {
    state.communityStickerPackOpen =
      typeof forceOpen === "boolean"
        ? forceOpen
        : !state.communityStickerPackOpen;
    renderAppShell();
  ...
  function selectCommunitySticker(sticker) {
    state.communitySelectedSticker =
      state.communitySelectedSticker === sticker ? "" : sticker;
  ...
  function handleCommunityFolderSelect(event) {
    ...
    state.communityAttachment = {
      ...
      files: includedFiles.map((file) => ({
        file,
        name: file.name,
        relativePath: normalizeFolderRelativePath(
  ...
  async function prepareCommunityAttachmentPayload(user, track, attachment) {
    ...
    if (attachment.kind === "folder") {
      return prepareCommunityFolderPayload(
        user,
        track,
        attachment,
    ...
    if (!communitySupabase) {
      if (attachment.size > MAX_LOCAL_ATTACHMENT_BYTES) {
        throw new Error(
          "Local file sharing only supports files up to 50 MB when community storage is unavailable.",
        );
      }
  ...
  async function buildCommunityFolderArchive(attachment) {
    if (!window.JSZip) {
      throw new Error(
        "Zip support is not loaded yet. Refresh the page and try again.",
      );
    }
    ...
    for (const fileEntry of attachment.files || []) {
      ...
      const relativePath = sanitizeFolderRelativePath(
    ...
    const archiveBaseName = sanitizeFileName(
      attachment.name || "shared-folder",
    );
    return new File([blob], `${archiveBaseName}.zip`, {
      type: "application/zip",
    });
  }

  // Folder uploads follow the same storage pipeline as normal files after we archive them.
  async function prepareCommunityFolderPayload(
    user,
    track,
    attachment,
    maxLocalBytes,
    ...
    if (!communitySupabase) {
      if (archiveFile.size > maxLocalBytes) {
        throw new Error(
          "Folder sharing without Supabase storage is limited to zip archives up to 50 MB.",
        );
      }
  ...
  async function postCommunityMessage() {
    ...
    if (!body && !sticker && !attachment) {
      state.communityComposerMessage = {
        type: "error",
        text: "Add a message, a sticker, a file, or a folder before sending.",
      };
      renderAppShell();
      return;
    }
    ...
    if (attachment) {
      ...
      try {
        attachmentPayload = await prepareCommunityAttachmentPayload(
          user,
          track,
          attachment,
        );
    ...
    const payload = {
      ...
      room: track.id,
      content: serializeCommunityPayload({
        body,
        sticker,
        attachment: attachmentPayload,
      }),
    ...
    if (communitySupabase) {
      ...
      if (error) {
        ...
        state.communityComposerMessage = {
          type: "error",
          text: "Message could not be sent to the shared room.",
        };
        renderAppShell();
        return;
      } else {
      ...
      const localMessages = readStoredCommunityMessages().filter(
        (message) =>
          !(
            ...
            (message.authorId === user.id || message.authorEmail === user.email)
          ),
      );
      writeStoredCommunityMessages(localMessages);
    }
  ...
  function toggleCurriculumSemester(semesterId) {
    state.currentCurriculumSemesterId =
      state.currentCurriculumSemesterId === semesterId ? null : semesterId;
    state.currentCurriculumMonthId = null;
  ...
  function toggleCurriculumMonth(monthId) {
    state.currentCurriculumMonthId =
      state.currentCurriculumMonthId === monthId ? null : monthId;
    const track = getCurrentTrack();
    if (track && state.currentCurriculumMonthId) {
      const lessonContext = track.semesters
        .map((semester) => ({
          semester,
          month: semester.months.find((item) => item.id === monthId),
        }))
  ...
  function toggleLessonCompletion(lessonId) {
    ...
    user.completedLessonIds = hasCompleted
  ...
  function toggleResourcesSemester(semesterId) {
    state.currentResourcesSemesterId =
      state.currentResourcesSemesterId === semesterId ? null : semesterId;
  ...
  function toggleLiveClassAttendance(classId) {
    ...
    user.joinedClassIds = joined
      ? user.joinedClassIds.filter((id) => id !== classId)
      : [...user.joinedClassIds, classId];
    void persistUsers();
    state.currentLiveClassId = classId;
    state.currentView = "live";
    renderAppShell();
  ...
  function shouldExcludeFolderEntry(path) {
    ...
    if (
      segments.some(
        (segment) =>
          shouldExcludeFolderName(segment) ||
          excludedFolders.has(segment.toLowerCase()),
      )
    ) {
      return true;
    }
  ...
  function getSearchContext() {
    ...
    const resolvedTrack = track
      ? { ...track, announcements: getTrackAnnouncements(track) }
      : track;
    return {
      user,
      track: resolvedTrack,
      communityMessages: state.communityMessages.filter(
        (message) => !track || message.trackId === track.id,
      ),
      openView: openApp,
      openLesson,
