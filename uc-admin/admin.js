(function () {
  ...
  async function initAdmin() {
    ...
    if (!supabaseClient) {
      showMessage(
        dom.adminAuthMessage,
        "Supabase client could not be initialized for admin tools.",
        "error",
      );
      return;
    }
  ...
  function bindDom() {
    ...
    dom.adminAnnouncementMessage = document.getElementById(
    ...
    dom.deleteFilteredMessagesBtn = document.getElementById(
      "deleteFilteredMessagesBtn",
    );
    dom.deleteAllMessagesBtn = document.getElementById("deleteAllMessagesBtn");
    dom.adminCommunityMessage = document.getElementById(
    ...
    dom.curriculumTrackSelect = document.getElementById(
      "curriculumTrackSelect",
    );
    dom.curriculumSemesterSelect = document.getElementById(
      "curriculumSemesterSelect",
    );
    dom.curriculumMonthSelect = document.getElementById(
      "curriculumMonthSelect",
    );
    ...
    dom.curriculumMonthSummary = document.getElementById(
      "curriculumMonthSummary",
    );
    dom.curriculumMonthPhase = document.getElementById("curriculumMonthPhase");
    dom.resetMonthOverrideBtn = document.getElementById(
      "resetMonthOverrideBtn",
    );
    ...
    dom.curriculumWeekObjective = document.getElementById(
      "curriculumWeekObjective",
    );
    dom.curriculumWeekType = document.getElementById("curriculumWeekType");
    dom.curriculumWeekVideoUrls = document.getElementById(
      "curriculumWeekVideoUrls",
    );
    dom.curriculumWeekResources = document.getElementById(
      "curriculumWeekResources",
    );
    dom.resetWeekOverrideBtn = document.getElementById("resetWeekOverrideBtn");
    dom.adminCurriculumMessage = document.getElementById(
      "adminCurriculumMessage",
    );
    ...
    dom.resourceSemesterSelect = document.getElementById(
      "resourceSemesterSelect",
    );
    dom.semesterResourcesForm = document.getElementById(
      "semesterResourcesForm",
    );
    dom.semesterResourceLinks = document.getElementById(
      "semesterResourceLinks",
    );
    dom.resetSemesterResourcesBtn = document.getElementById(
      "resetSemesterResourcesBtn",
    );
    dom.adminResourcesMessage = document.getElementById(
  ...
  function bindEvents() {
    ...
    dom.deleteFilteredMessagesBtn.addEventListener(
      "click",
      deleteFilteredMessages,
    );
    ...
    dom.curriculumTrackSelect.addEventListener(
      "change",
      handleCurriculumTrackChange,
    );
    dom.curriculumSemesterSelect.addEventListener(
      "change",
      handleCurriculumSemesterChange,
    );
    dom.curriculumMonthSelect.addEventListener(
      "change",
      handleCurriculumMonthChange,
    );
    dom.curriculumWeekSelect.addEventListener(
      "change",
      handleCurriculumWeekChange,
    );
    ...
    dom.resourceTrackSelect.addEventListener(
      "change",
      handleResourceTrackChange,
    );
    dom.resourceSemesterSelect.addEventListener(
      "change",
      handleResourceSemesterChange,
    );
    dom.semesterResourcesForm.addEventListener("submit", saveSemesterResources);
    dom.resetSemesterResourcesBtn.addEventListener(
      "click",
      resetSemesterResources,
    );
  ...
  async function handleAdminLogin(event) {
    ...
    if (error) {
      showMessage(
        dom.adminAuthMessage,
        await getAdminAuthErrorMessage(error),
        "error",
      );
      return;
    }
    ...
    if (!verified) {
      ...
      showMessage(
        dom.adminAuthMessage,
        "This authenticated account is not registered as an LMS admin.",
        "error",
      );
    }
  }
  ...
  async function verifyAdminSession() {
    ...
    const { data: isAdmin, error: adminError } =
      await supabaseClient.rpc("is_lms_admin");
    ...
    try {
      ...
      if (!records.length) {
        dom.logsStatus.textContent =
          "No logs were returned from the logging endpoint.";
        dom.systemLogsList.innerHTML =
          '<article class="empty-state">No log entries are available yet.</article>';
        return;
      }
      ...
      dom.logsStatus.textContent =
        "Logs are currently unavailable. Please try again shortly.";
  ...
  function renderLogCard(entry) {
    ...
    const severity =
      type.includes("error") || type.includes("crash")
        ? "error"
        : type.includes("warn") || type.includes("slow")
          ? "warning"
          : "info";
    const message = escapeHtml(
      String(entry?.message || entry?.error || "No message recorded"),
    );
    const location = escapeHtml(
      String(entry?.file || entry?.source || "Unknown script location"),
    );
    const line = entry?.line ? `Line ${entry.line}` : "Line unavailable";
    const pageUrl = escapeHtml(
      String(entry?.url || entry?.page || entry?.pathname || "Unknown page"),
    );
    const timestamp = escapeHtml(
      String(entry?.timestamp || entry?.created_at || "Unknown time"),
    );
    const width = entry?.viewport?.width || 0;
    const height = entry?.viewport?.height || 0;
    const resolution = escapeHtml(`${width} × ${height}`);
    const deviceType = escapeHtml(
      String(
        entry?.viewport?.deviceType ||
          entry?.deviceType ||
  ...
  async function deleteLogAndHideCard(logId, button) {
    ...
    if (!deleted) {
      ...
      dom.logsStatus.textContent =
        "Failed to delete the selected log. Please try again.";
      return;
    }
  ...
  async function clearAllLogs() {
    ...
    dom.systemLogsList.innerHTML =
      '<article class="empty-state">All logs have been cleared.</article>';
  ...
  async function fetchFeedback() {
    const { data, error } = await supabaseClient
      .from(FEEDBACK_TABLE)
      .select(
        "id, user_email, user_name, track_id, category, message, image_urls, created_at",
      )
      .order("created_at", { ascending: false });
  ...
  async function fetchPublicProfiles() {
    const { data, error } = await supabaseClient
      ...
      .select(
        "email, first_name, last_name, track_id, timezone, headline, bio, avatar_url, is_active, managed_note, updated_at",
      )
  ...
  async function fetchCurriculumOverrides() {
    const [
      ...
      supabaseClient
        ...
        .select(
          "month_id, track_id, semester_id, label, title, summary, phase",
        ),
      supabaseClient
        .from(WEEK_OVERRIDES_TABLE)
        .select(
          "week_id, track_id, semester_id, month_id, title, objective, type, video_url, video_urls, resources, resource_items",
        ),
    ]);
    ...
    state.monthOverridesById = Object.fromEntries(
      (monthData || []).map((item) => [
        item.month_id,
        {
          trackId: item.track_id,
    ...
    state.weekOverridesById = Object.fromEntries(
      (weekData || []).map((item) => [
        item.week_id,
        {
          trackId: item.track_id,
  ...
  function getResolvedTrack(trackId) {
    ...
    const effectiveSemesterCount = Number.isFinite(settings.semesterCount)
    ...
    if (
      ...
      baseTrack = buildFallbackTrack(trackId, settings);
    }
    ...
    return {
      ...
      outcomes:
        Array.isArray(settings.outcomes) && settings.outcomes.length
          ? settings.outcomes
          : baseTrack.outcomes,
          ...
          return {
            ...
            weeks: month.weeks.map((week) => {
              ...
              return {
                ...
                videoUrl:
                  weekOverride.videoUrls?.[0] ||
                  weekOverride.videoUrl ||
                  week.videoUrl ||
                  "",
                videoUrls:
                  Array.isArray(weekOverride.videoUrls) &&
                  weekOverride.videoUrls.length
                ...
                resourceItems:
                  ...
                  weekOverride.resourceItems.length
                    ...
                    : Array.isArray(weekOverride.resources) &&
                      ...
                      ? weekOverride.resources.map((item) => ({
                          title: item,
                          url: "",
                          kind: "resource",
                        }))
  ...
  function ensureSelectedUser() {
    ...
    if (
      !filteredUsers.some(
        (profile) => profile.email === state.selectedUserEmail,
      )
    ) {
  ...
  function getFilteredUsers() {
    return state.publicProfiles.filter((profile) => {
      return (
        state.userTrackFilter === "all" ||
        profile.trackId === state.userTrackFilter
      );
    });
  }
  ...
  function renderOverview() {
    const tracks = getResolvedTracks();
    const totalSemesters = tracks.reduce(
      (sum, track) => sum + track.semesters.length,
      0,
    );
  ...
  function renderAnnouncements() {
    if (!state.announcements.length) {
      dom.announcementList.innerHTML =
        '<div class="empty-state">No admin announcement has been published yet.</div>';
      return;
    }

    dom.announcementList.innerHTML = state.announcements
      .map(
        (item) => `
  ...
  function renderMessages() {
    const filteredMessages = state.messages.filter((message) => {
      return (
        state.messageTrackFilter === "all" ||
        message.trackId === state.messageTrackFilter
      );
    });

    if (!filteredMessages.length) {
      dom.messageList.innerHTML =
        '<div class="empty-state">No message matched the current moderation filter.</div>';
      return;
    }

    dom.messageList.innerHTML = filteredMessages
      .map(
        (message) => `
      <article class="admin-list-item">
        <div class="admin-list-meta">
  ...
  function renderBookList() {
    ...
    if (!books.length) {
      dom.bookList.innerHTML =
        '<div class="empty-state">No books have been created yet. Add one above to show it on the learner dashboard.</div>';
      return;
    }
  ...
  function renderFeedbackList() {
    if (!state.feedbackItems.length) {
      dom.feedbackList.innerHTML =
        '<div class="empty-state">No learner feedback has been submitted yet.</div>';
      return;
    }

    dom.feedbackList.innerHTML = state.feedbackItems
      .map(
        (item) => `
      <article class="admin-list-item">
        <div class="admin-list-meta">
  ...
  async function handleBookSubmit(event) {
    ...
    if (!title || !link || !summary) {
      showMessage(
        dom.adminBookMessage,
        "Book title, description, and link are required.",
        "error",
      );
      return;
    }
    ...
    try {
      ...
      const { error } = await supabaseClient.from(BOOKS_TABLE).insert({
        title,
        summary,
        link,
        image_url: imageUrl || "",
      ...
      showMessage(
        dom.adminBookMessage,
        "Book card saved locally because remote sync is unavailable right now.",
        "success",
      );
      window.dispatchEvent(new CustomEvent("rkh-books-updated"));
    } finally {
  ...
  window.deleteAdminBook = async function (bookId) {
    try {
      ...
      const books = readBooksCatalog().filter(
      ...
      showMessage(
        dom.adminBookMessage,
        "Book card deleted successfully.",
        "success",
      );
      window.dispatchEvent(new CustomEvent("rkh-books-updated"));
    } catch (error) {
      ...
      showMessage(
        dom.adminBookMessage,
        "Remote deletion could not be completed. Please check your connection.",
        "error",
      );
    }
  };
  ...
  function renderTrackSettings() {
    const tracks = getResolvedTracks();
    dom.trackSettingsList.innerHTML = tracks
      .map((track) => {
  ...
  function ensureCurriculumSelection() {
    const track =
      getResolvedTrack(state.curriculumTrackId) ||
      getResolvedTracks()[0] ||
      null;
    if (!track) return;
    ...
    const semester =
      track.semesters.find((item) => item.id === state.curriculumSemesterId) ||
      track.semesters[0] ||
      null;
    if (!semester) return;
    ...
    const month =
      semester.months.find((item) => item.id === state.curriculumMonthId) ||
      semester.months[0] ||
      null;
    if (!month) return;
    ...
    const week =
      month.weeks.find((item) => item.id === state.curriculumWeekId) ||
      month.weeks[0] ||
      null;
  ...
  function renderCurriculumSelectors() {
    ...
    dom.curriculumSemesterSelect.innerHTML = track.semesters
      .map(
        (semester) =>
          `<option value="${semester.id}">${escapeHtml(`${semester.label} - ${semester.title}`)}</option>`,
      )
      .join("");
    dom.curriculumSemesterSelect.value = state.curriculumSemesterId;

    const semester = track.semesters.find(
      (item) => item.id === state.curriculumSemesterId,
    );
    const months = semester?.months || [];
    dom.curriculumMonthSelect.innerHTML = months
      .map(
        (month) =>
          `<option value="${month.id}">${escapeHtml(`${month.label} - ${month.title}`)}</option>`,
      )
      .join("");
    ...
    dom.curriculumWeekSelect.innerHTML = weeks
      .map(
        (week) =>
          `<option value="${week.id}">${escapeHtml(week.title)}</option>`,
      )
      .join("");
  ...
  function renderCurriculumForms() {
    ...
    const month =
      ...
      null;
    const week =
      month?.weeks.find((item) => item.id === state.curriculumWeekId) || null;
    ...
    const weekOverride =
      state.weekOverridesById[state.curriculumWeekId] || null;
    const weekVideoUrls = weekOverride
      ...
      : [];
    const weekResourceLines =
      Array.isArray(week?.resourceItems) && week.resourceItems.length
        ? week.resourceItems.map((item) =>
  ...
  function ensureResourceSelection() {
    const track =
      getResolvedTrack(state.resourceTrackId) || getResolvedTracks()[0] || null;
    if (!track) return;
    state.resourceTrackId = track.id;

    const semester =
      track.semesters.find((item) => item.id === state.resourceSemesterId) ||
      track.semesters[0] ||
      null;
  ...
  function renderResourcesEditor() {
    ...
    const links =
      state.semesterResourcesByKey[
        `${track.id}::${state.resourceSemesterId}`
      ] || [];
  ...
  function renderUsers() {
    ...
    if (!filteredUsers.length) {
      dom.userList.innerHTML =
        '<div class="empty-state">No learner profile has been synced for this filter yet.</div>';
      dom.userForm.reset();
      return;
    }

    dom.userList.innerHTML = filteredUsers
      .map(
        (profile) => `
    ...
    dom.userTrack.value =
      selectedProfile.trackId || Object.keys(window.RKH_DATA.tracks)[0] || "";
  ...
  async function handleAnnouncementSubmit(event) {
    ...
    if (!trackId || !title || !body) {
      showMessage(
        dom.adminAnnouncementMessage,
        "Track, title, and body are required.",
        "error",
      );
      return;
    }
  ...
  async function saveMonthOverride(event) {
    ...
    if (!track || !semester || !month) {
      showMessage(
        dom.adminCurriculumMessage,
        "Select a valid track, semester, and month before saving.",
        "error",
      );
      return;
    }
    ...
    showMessage(
      dom.adminCurriculumMessage,
      "Month override saved successfully.",
      "success",
    );
  ...
  async function saveWeekOverride(event) {
    ...
    if (!track || !semester || !month || !week) {
      showMessage(
        dom.adminCurriculumMessage,
        "Select a valid track, semester, month, and week before saving.",
        "error",
      );
      return;
    }
    ...
    showMessage(
      dom.adminCurriculumMessage,
      "Week override saved successfully.",
      "success",
    );
    await fetchCurriculumOverrides();
    renderCurriculumEditor();
  ...
  async function resetMonthOverride() {
    ...
    const shouldReset = window.confirm(
      "Reset this month back to the base curriculum content?",
    );
  ...
  async function resetWeekOverride() {
    ...
    const shouldReset = window.confirm(
      "Reset this week back to the base curriculum content?",
    );
    if (!shouldReset) return;
  ...
  async function saveSemesterResources(event) {
    ...
    if (!track || !semester) {
      showMessage(
        dom.adminResourcesMessage,
        "Select a valid track and semester before saving.",
        "error",
      );
      return;
    }
    ...
    showMessage(
      dom.adminResourcesMessage,
      "Semester resources saved successfully.",
      "success",
    );
  ...
  async function resetSemesterResources() {
    ...
    const shouldReset = window.confirm(
      "Reset the resource links for this semester?",
    );
    if (!shouldReset) return;
  ...
  async function createTrackFromAdmin(event) {
    ...
    const rawId = (
      dom.newTrackId.value ||
      dom.newTrackLabel.value ||
      ""
    ).trim();
    const trackId = rawId
      .toLowerCase()
    ...
    if (!trackId || !label) {
      showMessage(
        dom.adminTrackMessage,
        "Enter both a track ID and a display name.",
        "error",
      );
      return;
    }
    ...
    if (!window.RKH_DATA.tracks[trackId]) {
      window.RKH_DATA.tracks[trackId] = buildFallbackTrack(trackId, {
        label,
        summary,
        outcomes,
        semesterCount,
  ...
  function buildFallbackTrack(trackId, settings = {}) {
    ...
    return {
      ...
      summary:
        settings.summary || "Custom track created from the admin dashboard.",
      outcomes:
        Array.isArray(settings.outcomes) && settings.outcomes.length
          ? settings.outcomes
          : ["Track outcomes"],
  ...
  async function saveTrackSettings(trackId) {
    const baseTrack =
      window.RKH_DATA.tracks[trackId] ||
      buildFallbackTrack(trackId, state.trackSettingsById[trackId] || {});
    if (!baseTrack) return;

    const label =
      document.getElementById(`track-label-${trackId}`)?.value.trim() ||
      baseTrack.label;
    const summary =
      document.getElementById(`track-summary-${trackId}`)?.value.trim() ||
      baseTrack.summary;
    const outcomes = (
      document.getElementById(`track-outcomes-${trackId}`)?.value || ""
    )
      .split(/\r?\n/)
      .map((item) => item.trim())
      .filter(Boolean);
    const sortOrder = Number(
      document.getElementById(`track-sort-${trackId}`)?.value || 0,
    );
    const semesterCount = Number(
      document.getElementById(`track-semesters-${trackId}`)?.value || 3,
    );
    const isEnabled =
      document.getElementById(`track-enabled-${trackId}`)?.value !== "false";
    ...
    showMessage(
      dom.adminTrackMessage,
      "Track settings saved successfully.",
      "success",
    );
    await fetchTrackSettings();
    renderTrackSettings();
  ...
  async function resetTrackSettings(trackId) {
    const shouldReset = window.confirm(
      "Reset this track back to the base LMS configuration?",
    );
    if (!shouldReset) return;
  ...
  async function deleteTrack(trackId) {
    ...
    if (!state.trackSettingsById[trackId]) {
      showMessage(
        dom.adminTrackMessage,
        "Only custom tracks created from the admin dashboard can be deleted here.",
        "error",
      );
      return;
    }
    ...
    const shouldDelete = window.confirm(
      `Delete custom track "${trackLabel}"? This removes its admin override and local fallback entry.`,
    );
  ...
  async function deleteSelectedUserProfile() {
    if (!state.selectedUserEmail) {
      showMessage(
        dom.adminUserMessage,
        "Select a learner profile before deleting it.",
        "error",
      );
      return;
    }

    const profile = state.publicProfiles.find(
      (item) => item.email === state.selectedUserEmail,
    );
    const label = profile
      ? `${profile.firstName} ${profile.lastName}`.trim() || profile.email
      : state.selectedUserEmail;
    const shouldDelete = window.confirm(
      `Delete learner profile for ${label}? This removes the record from the admin learner list.`,
    );
    if (!shouldDelete) return;
  ...
  async function saveUserProfile(event) {
    ...
    if (!state.selectedUserEmail) {
      showMessage(
        dom.adminUserMessage,
        "Select a learner profile before saving.",
        "error",
      );
      return;
    }
  ...
  async function deleteFeedback(id) {
    ...
    const label = entry
      ...
      : "this feedback entry";
    const shouldDelete = window.confirm(
      `Delete feedback from ${label}? This can only be done by an admin.`,
    );
    if (!shouldDelete) return;
  ...
  async function deleteFilteredMessages() {
    ...
    if (!filteredMessages.length) {
      showMessage(
        dom.adminCommunityMessage,
        "There are no messages to delete for this filter.",
        "error",
      );
      return;
    }
    ...
    showMessage(
      dom.adminCommunityMessage,
      "Filtered messages deleted successfully.",
      "success",
    );
    await fetchMessages();
    renderOverview();
  ...
  async function deleteAllMessages() {
    const shouldDelete = window.confirm(
      "Delete every learner message across all tracks? This cannot be undone.",
    );
    if (!shouldDelete) return;
